...
- 对象不再被应用程序使用（即无用），但仍有可达引用链，导致 GC 无法回收，从而长期占用内存。
...
- Lambda表达式是简化匿名内部类（它只能替换函数式接口，即只有一个抽象方法的接口）的语法。
...
- `abstract`用于定义抽象类和抽象方法。（抽象类中可以没有抽象方法）
- 抽象类不能实例化，普通类可以。
...
1. 类 final（防止继承篡改）。
2. 所有字段 private final（保证引用不可变）。
3. 不提供 setter（没有修改状态的方法）。
4. 构造器进行防御性拷贝（避免外部持有可变对象的引用）。
5. Getter 返回防御性拷贝或不可变视图（避免内部字段逃逸）
...
**答案**：
以上**代码不能编译**。因为所有类对继承根对象类Object。以上会编译错误提示静态方法不能覆盖实例方法。静态方法不能隐藏父类的实例方法，同样一个类中不能定义同名且同参数的静态方法和实例方法
...
**答案**：
值传递
[示例37](https://github.com/jesshaw/java-sample/blob/master/interview/src/main/java/com/lexiangmiao/sample/interview/JavaFundamentals/JF_37_PassByValueDemo.java)
