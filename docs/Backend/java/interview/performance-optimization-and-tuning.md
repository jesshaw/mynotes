...
- **内存分析**：使用jmap、VisualVM、jstat等工具分析内存使用情况，定位可能的内存泄漏或GC频繁问题。
...
## 四. 系统架构上的性能优化措施（缓存、CDN、数据库读写分离等）
...
## 六. Java 程序出现 CPU 使用率周期性上升和下降如何排查该问题？

排查 CPU 周期性波动，核心思路就两点：**在峰顶抓住肇事线程，再从周期特征反推触发机制。**

---

1.  **量化周期**  
    `top -Hp <pid>` 观察线程 CPU 波动，记下间隔（如 30 秒）。
2.  **排除 GC**  
    `jstat -gcutil <pid> 1000` 看是否 Minor/Full GC 频繁。若是，分析堆和分配速率。
3.  **峰顶抓栈**  
    高峰时连续 `jstack <pid> > stack.log` 2~3 次，对比栈顶 `RUNNABLE` 线程。  
    或直接用 Arthas：`thread -n 3` 重复执行。
4.  **对号入座**  
    根据线程名 + 堆栈定位：
    - `GC` 线程 → 内存问题
    - `scheduling-*`、`pool-*-thread-*` → 业务定时任务
    - `Druid-*`、`HikariCP-*` → 连接池心跳/清理
    - `nioEventLoop-*`、`consumer-*` → 框架/中间件心跳
    - `*cache-refresh*` → 本地缓存重建
5.  **火眼金睛（可选）**  
    若堆栈不清，用 `async-profiler -e cpu -d 30 -f flame.html <pid>` 生成火焰图，找定时循环逻辑。
...
| 命令 | 作用 |
|------|------|
| `top -Hp <pid>` | 查看进程内每个线程的 CPU 使用率，锁定峰顶线程 ID |
| `jstat -gcutil <pid> 1000` | 实时看 GC 频率与耗时，判断是否 GC 导致的周期 |
| `jstack <pid> > log` （高峰连抓多次）| 获取线程栈，定位 `RUNNABLE` 线程的调用链 |
| `jstack <pid> \| grep -A20 0x<nid>` | 根据 `top` 得到的线程 ID (转十六进制) 直查堆栈 |
| `Arthas: thread -n 3` | 快速列出 CPU 最高的线程及栈，无需解析 nid |
| `async-profiler: ./profiler.sh -e cpu -d 30 -f /tmp/fg.html <pid>` | 生成火焰图，精确到方法级热点 |
| `sar -u 1` / 监控系统 | 量化周期长度，辅助验证 |

**一招定乾坤**：周期一到，`top -Hp` 看到高 CPU 线程，记下 PID，`jstack` 搜索 `0x<十六进制PID>`，代码立现。

## 七. Java应用中，内存一直居高不下，如何排查解决？

针对 Java 内存居高不下，精简的排查思路和命令如下：

### 1. 判断类型
**思路：先确定是堆内存还是堆外内存。**
- `top -Hp <pid>` 看 `RES`，`jstat -gcutil <pid> 1000` 看堆使用率  
  - 堆使用率极高 + 频繁 GC → 堆内问题  
  - `RES` 高但堆使用率正常 → 堆外问题
...
# 深度分析：dump（会 STW，生产谨慎）
jmap -dump:live,format=b,file=heap.hprof <pid>
# 然后用 MAT 查 Dominator Tree → Path to GC Roots
```

### 3. 堆外内存排查
需提前添加 JVM 参数：`-XX:NativeMemoryTracking=detail`
```bash
# 查看堆外内存分布（含 Thread/Internal/Class）
jcmd <pid> VM.native_memory summary

# 查看进程内存映射细节
pmap -x <pid> | sort -nrk3 | head

# 查 DirectByteBuffer 实例（Arthas）
...
# 堆外 DirectBuffer 泄漏：检查 NMT 的 Internal 项是否持续增长
```

### 5. 对应行动
- 对象泄漏：MAT 分析引用链，清理静态集合、ThreadLocal 等
- 线程膨胀：约束线程池，减小 `-Xss`
- Metaspace 满：检查动态代理/CGLIB，增大 `-XX:MaxMetaspaceSize`
- DirectByteBuffer 泄漏：确保 finally 中 `cleaner().clean()`，或排查 Netty 等框架的 ByteBuf 释放

## 八. MySQL查询时突然变慢，如何排查并解决？
...
- 常用工具：
  ...
  - **Spring Root**：提供命令行工具，可以快速生成Spring Boot项目的代码骨架。
