...
- **Raft**（首选，易理解）和 **Paxos**（理论基石）：通过“多数派（Quorum）写入”和“日志复制”来保证集群在节点宕机时仍返回最新数据。
- *（注：2PC仅解决事务原子性且易阻塞，3PC理论化几乎不用，工业界已抛弃。）*

**2. 最终一致性（高并发场景）**
依靠**柔性事务**，牺牲瞬时一致性换取高可用。

- **TCC**（预留-确认-取消）或 **Saga**（长事务补偿）：配合**可靠消息队列（如RocketMQ）**，通过异步重试和回滚，保证数据在短时间内最终对齐。

**3. 核心理论边界（CAP定理）**

- **关键纠正**：**仅在“发生网络分区（P）”时**，才被迫在一致性（C）和可用性（A）之间二选一。**未分区时，系统CA可以兼得**。
- 选 **CP**（如ZooKeeper）：分区时宁可停服，也要保数据绝对一致。
- 选 **AP**（如Eureka/Cassandra）：分区时宁可返回旧数据，也要保服务不停。
