...
[源码示例](https://github.com/jesshaw/java-sample/blob/master/interview/src/main/java/com/lexiangmiao/sample/interview/MultithreadingAndConcurrency/MAC_2.java)
...
**答案**：
- `volatile`**保证了变量的可见性**，即当一个线程修改了变量的值，其他线程立即可见；。`volatile`不会保证操作的原子性，所以仅在**非复合操作**（赋值右边是变量的操作）的共享变量使用。
- volatile 的**有序性**，本质上是禁止了 JVM 和 CPU 对指令的优化重排。它通过内存屏障，确保了‘前面的操作一定先于后面的操作发生’，防止多线程环境下拿到未初始化的对象或过期的上下文数据。
...
- 从Java应用层（开发者视角）看：默认是单线程的。
- 从JVM底层（操作系统/运行时视角）看：是多线程的。
